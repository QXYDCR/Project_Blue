...
class NetMLP(nn.Module):
    ...
    def compute_loss(self, x, y):
        ...
        accuracy, precision, recall, f1 = [metric(y.cpu(), outputs.cpu()) for metric
            ...
            if epoch % freq_prints == 0:
                print(
                    ...
                    "Val   performance: loss={:.3f}, accuracy={:.3f}, precision={:.3f}, recall={:.3f}, f1={:.3f}".format(
                        epoch + 1, self.epochs, history['loss'][-1], history['accuracy'][-1], history['precision'][-1],
                        history['recall'][-1], history['f1'][-1],
                        history['val_loss'][-1], history['val_accuracy'][-1], history['val_precision'][-1],
                        history['val_recall'][-1], history['val_f1'][-1]
                    ))
...
class NetCNN(nn.Module):
    ...
    def __init__(self, vocab_size, embedding_matrix, filter_sizes=[1, 2, 3, 4, 5],
                 num_filters=16, embed_size=200, finetune_emb=False, epochs=5, learning_rate=0.001, l2reg=1e-4,
                 dropout=0.1):
        '''
            Constructor for neural network; defines all layers and sets attributes for optimization.
        '''
        ...
        self.convs1 = nn.ModuleList([nn.Conv2d(1, self.num_filters, (K, self.embed_size))
    ...
    def compute_loss(self, x, y):
        ...
        # binarize predictions from predictions (outputs = 1 if p>0.5 else 0)
        outputs = (predictions > 0.5).float()
        accuracy, precision, recall, f1 = [metric(y.cpu(), outputs.cpu()) for metric in
